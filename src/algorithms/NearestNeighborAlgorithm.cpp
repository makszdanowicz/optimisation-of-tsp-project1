//
// Created by Lenovo on 17.10.2024.
//

#include "../../include/NearestNeighborAlgorithm.h"
#include <vector>
#include <iostream>

using namespace std;

NearestNeighborAlgorithm::NearestNeighborAlgorithm(Matrix matrix): matrix(matrix) {}

void NearestNeighborAlgorithm::algorithmSolve() {
    int sizeOfOfVertices = matrix.getSize();
    int bestCost = INT_MAX; // value to find the minimum cost amount each vertex
    // Checking the minimal cost for travelling for each vertex
    for(int startVertex = 0; startVertex < sizeOfOfVertices; startVertex++){
        vector<bool> visitedVertices(sizeOfOfVertices, false); // vector of vertices, that stores information which vertex was visited
        int totalCost = 0;
        int currentVertex = startVertex;
        visitedVertices[currentVertex] = true; // adding current vertex to list of visited vertices
        // checking costs of unvisited neighbors of visitedVertex to find closest neighbor
        for(int amountOfVerticesToVisit = 1; amountOfVerticesToVisit < sizeOfOfVertices; amountOfVerticesToVisit++){
            int minCost = INT_MAX;
            int nearestNeighbor = -1;
            for(int neighbor = 0; neighbor < sizeOfOfVertices; neighbor++){
                int cost = matrix.getCost(currentVertex,neighbor);
                // checking is it currentCost is the shortest cost to neighbor for current vertex
                if(!visitedVertices[neighbor] && cost > -1 && cost < minCost){
                    minCost = cost;
                    nearestNeighbor = neighbor;
                }
            }
            // if there is the closest neighbor, add information about it path
            if(nearestNeighbor != -1){
                visitedVertices[nearestNeighbor] = true;
                totalCost += minCost;
                currentVertex = nearestNeighbor;
            }
        }
        // add cost of returning path to total cost
        totalCost += matrix.getCost(currentVertex,startVertex);

        // add total cost of current path to best cost
        if(totalCost < bestCost){
            bestCost = totalCost;
        }
    }
    cout << "The best cost: " << bestCost << endl;

}
